...
%     
%     %LR分类
%     model = classf_knn_tr(train_data,train_label);
%     %LR网络预测
%     [predict_label] = classf_knn_te(model,test_data);
...
for i=1:20 % 运行20次取平均值
    ...
    %svm分类
    model = classf_knn_tr(xapp,yapp);
    % SVM网络预测
    [predict_label] = classf_knn_te(model,xtest);
